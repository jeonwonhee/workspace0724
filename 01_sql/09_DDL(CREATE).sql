/*/*
    ...
    즉, 실제 데이터값이 아니라 규칙 자체를 정의하는 언어
    
    오라클에서 객체(구조) : 테이블, 뷰, 시퀀스, 인덱스, 패키지, 트리거, 프로시져, 함수, 사용자 등등...
...
/*
    ...
    1. 테이블 생성
    -테이블 : 행과 열로 구성하는 가장 기본적인 데이터베이스 객체
    ...
    *자료형
    ...
    - 문자(CHAR(바이트크기) | VARCHAR2(바이트크기)) -> 반드시 크기를 지정해야 함
        ...
        - VARCHAR2 : 최대 40000바이트까지 지정가능 / 가변길이(몇글자의 데이터가 들어올지 확신할 수 없는 경우)
    - 날짜(DATE) 
...
CREATE TABLE MEMBER(
       MEM_NO NUMBER, 
       MEM_ID VARCHAR2(30), --USER01, TEST1235
       MEM_PWD VARCHAR2(30),
...
/*
    ...
    - 데이터 무결성보장을 목적으로 함 
    -> 데이터베이스에 저장된 데이터가 정확하고 일관되며 신뢰할 수 있는 상태를 유지
...
CREATE TABLE MEMBER(
       ...
       MEM_ID VARCHAR2(30) NOT NULL, --USER01, TEST1235
...
CREATE TABLE MEMBER(
       ...
       MEM_ID VARCHAR2(30)CONSTRAINT MEMID_NT NOT NULL, --USER01, TEST1235
       MEM_PWD VARCHAR2(30) CONSTRAINT MEMPWD_NT NOT NULL,
       MEM_NAME VARCHAR2(21) CONSTRAINT MEMNAME_NTNOT NULL,
       GENDER CHAR(3),
       PHONE VARCHAR2(13),
       EMAIL VARCHAR2(50),
       CREATE_AT DATE,
       CONSTRAINT MEMID_NT UNIQUE(MEM_ID)
);
...
--기본키에 중복된 값을 담으려고 할 때 (UINIQUE)제약조건 위반
...
CREATE TABLE MEM(
       ...
       MEM_ID VARCHAR2(30) NOT NULL UNIQUE , --USER01, TEST1235
       MEM_PWD VARCHAR2(30) NOT NULL,
       MEM_NAME VARCHAR2(21) NOT NULL,
       GENDER CHAR(3)CHECK(GENDER IN ('남','여')),
       PHONE VARCHAR2(13),
...
/*
    ...
    -> 다른 테이블을 참조한다고 표현
    -> 대부분의 경우는 FOREIG KEY를 통해서 테이블간의 관계가 활성화된다.
...
--유효하지 않은 회원등급번호는 정상적으로 INSERT할 수 없음.
INSERT INTO MEM
VALUES (5, 'USER5', 'PASS5', '홍길오', NULL, NULL, NULL, '25/01/01',50);
...
WHERE GRADE_CODE = 10;
--MEM테이블에서 10이라는 GRADE_ID가 있기 때문에 부모테이블의 GRADE_CODE 10을 가진
--데이터 삭제가 안됨
...
--부모테이블로부터 무조건 삭제가 안되는 "삭제제한" 옵션이 기본값으로 설정되어잇음.

--------------------------------------------------------------------------------
/*
    자섹테이블 생성시 외래키 제약조건 부여할 때 삭제옵션을 함께 지정가능.
    *삭제옵션: 부모테이블의 데이터 삭제시 그 데이터를 참조하고 있는 자식테이블의 데이터를
              어떻게 처리할 것인가?
...
DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 20;
--MEM_GRADE의 PK 40데이터가 삭제되면 SET_NULL 옵션에 따라 
--40인 데이터를 참조하던 자식데이터들은 전부 NULL을 가짐
...
--자식데이터 중 10인 데이터를 참조하던 자식데이터들은 전부 함께 삭제된다.
...
INSERT INTO MEM VALUES(3,'USER03',24,DEFAULT,DEFAULT);
...
--AS 뒤에 오는 SELECT문의 결과를 기반으로 테이블을 생성하겠다.
...
/*
    ...
    -FOREIGN KEY ; ALTER TABLE 테이블명 ADD [COMSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFERENCES 참조할 테이블명(컬럼명);
    -UNIQUE : ALTER TABLE 테이블명 ADD [COMSTRAINT 제약조건명] UNIQUE(컬럼명);
    -CHECK : ALTER TABLE 테이블명 ADD [COMSTRAINT 제약조건명] CHECK(컬럼에 대한 조건식);
