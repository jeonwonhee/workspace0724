...
public class MemberController {
	...
	public void memberSearch(String userId, String Keyword) {
		Member m = new Member();
		m.setUserId(userId);
		m.setUserId(Keyword);
		
		int result = ms.memberSearch(m);
		
		if(result > 0) {
			new MemberMenu().displaySuccess("키워드 검색에 성공하셨습니다.");
		} else {
			new MemberMenu().displayFail("키워드 검색에 실패하였습니다.");
		}
	}
}
