/*
    ...
    -그룹함수: N개의 값을 읽어들여서 1개의 결과값을 리턴(그룹을 지어 그룹별로 함수실행결과를 반환)
    
    >>SELECT절에 단일행 함수오 그룹함수를 함께 사용하지 못한다.
...
SELECT SUBSTR('SHOWMETHEMONRY',7) FROM DUAL; -- 7번 위치부터 끝까지
SELECT SUBSTR('SHOWMETHEMONRY',1,6) FROM DUAL; --1번부터 6번째까지
...
SELECT LTRIM('     K  H   ') FROM DUAL; --앞에서부터 다른 문자가 나올 때까지 공백을 제거ㄴ딛ㅊㅅ
...
SELECT TRIM(BOTH 'Z' FROM 'ZZZZZK  HZZZZZZ') FROM DUAL;
...
SELECT ROUND(123.456) FROM DUAL;  --위치를 입력하지 않으면 기본차수는 소수점 첫번째 자리에서 반올림
SELECT ROUND(123.456,1) FROM DUAL; --양수로 증가할 수록 소수점 뒤로 한칸씩 이동한다.
...
SELECT CEIL(123.456) FROM DUAL; --무조건 소수점 첫벉째에서 반올림
...
SELECT EMP_NAME, HIRE_DATE, SYSDATE - HIRE_DATE, MONTHS_BETWEEN(SYSDATE, HIRE_dATE)
FROM EMPLOYEE;
...
--사원데이블에서 사원명, 입사일, 수습기간 종료일(입사일로부터 3개월) 조회
...
/*
  EXTRACT : 특정 날짜로부터 년1월1일 값만 추출해서 반환
...
SELECT TO_DATE('20020505 120500', 'YYYYMMDD HHMISS') FROM DUAL;
...
SELECT '1000' + '5000' FROM DUAL; --문자열 -> 숫자로 자동형변환이 잔향ㄷ한더,
...
--NULLIF(비교대상1, 비교대상2)
--두 값이 일치하면 NULL 반환, 일치하지 않으면 비교대상1 반환
...
SELECT MIN(SALARY), MIN(HIRE_DATE)
FROM EMPLOYEE;
...
--5. COUNT(* | Z컬럼 | DISTINCT 컬럼 ) : 해당 조건에 맞는 행의 갯수를 세서 반환
--COUNT(*) : 조회된 결과에 모든 행의 갯수를 반환
--COUNT(컬럼) : 제시한 컬럼값이 NULL이 아닌 것만 행의 수를 세서 반환
--COUNT(DISTINCT컬럼) : 해당 컬럼값에서 중복을 제거한 후 행의 갯수를 세서 반환
...
SELECT COUNT(BONUS) --BONUS컬럼의 값이 NULL이 아닌 데이터만 카운트함
FROM EMPLOYEE;
