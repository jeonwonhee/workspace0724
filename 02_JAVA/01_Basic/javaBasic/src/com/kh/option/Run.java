 ...
 * 
 * static
 * static변수와 static메서드는 정적 메모리에 저장된다.
 * 프로그램 실행시점에 메모리가 생성되며, 객체생성없이도 접근이 가능하다
 * 모든 객체가 공유하여 사용한다.
 * 
 * static변수
 * 클래스에 소속된 모든 인스턴스가 공유하는 변수
 * 한 클래스에서 공통적인 앖을 유지해야할 때 사용
 * 객체생성없이 클래스명.변수명 형태로 접근이 가능하며 일반적으로 final과 함께 많이 사용됨
 * 
 * static메서드
 * 인스턴스 변수없이 클래스 단위로 호출할 수 있는 메서드
 * 객체 상태와 무관하게 동작하는 반복적인 기능 구현시 유용
 * 단, 인스턴스 변수나 인스턴스 메서드에 직접접근이 불가 -> this키워드 사용x
 * 
 * */
