/*
   ...
   => 테이블은 여러 컬럽으로 구성되고, 각 컬럼은 테이블이 표현하는 데이터의 세부적인 속성을 나타냄
...
--5 EMPLOYEE테이블에서 이름, 연봉, 총수령액(보너스 포함), 실수령액(총수령액-(연봉*세금(3%))
--NVL(A,N)은 A의 값이 NULL일때 N으로 대체한다.
-- AS ".." 는 속성이름을 지정해줌
...
--데이터베이스에서 날짜를 계산할 때 덧셈, 뺼셈이 가능하다
--현재시간-입사일=근무시간
--DATE-DATE => 결과를 무조건 일로 표시한다.
--시간으로 표시하고 싶으면 *24를 한다.
--코드 실행시 현재날짜를 표시하는 상수: SYSDATE[년/월/일/시/초]
...
SELECT EMP_NAME ||'님 급여는 ' || SALARY || '원 입니다.'
FROM EMPLOYEE;
...
/*실제로 사용되고 있는 직급 목록이 궁금할 때*/
SELECT DISTINCT JOB_CODE
FROM EMPLOYEE;

/*실제로 사용되고 있는 부서 목록이 궁금할 때*/
...
/*
     <WHERE절>
     조회하고자하는 테이블로부터 특정 조건에 만족하는 데이터만 조회하고자 할 때 사용한다.
...
--EMPLOYEE테이블에서 부서코드가 D9(총무부)인 사원들만 조회(모든 컬럼)
SELECT *
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9';
...
/*
   <BETWEEN AND>
    조건식에 사용되는 구문
    몇이상 몇이하의 범위에 대한 조건을 제시할 때 주로 사용하는 연산자(이상,이하만 가능)
    [표현법]
...
/*
    ...
    EX) 비교할 대상 컬럼 LIKE '%문자' : 비교할 대상컬럼 값 중에서 해당문자로 끝나는 값을 전부 조회.
    EX) 비교할 대상 컬럼 LIKE '%문자%' : 비교할 대상컬럼 값 중에서 해당문자가 포함된 값을 전부 조회.
...
--2. 전화번호 처음3자리가 010이 아닌 사원들의 사원명, 전화번호 조회
...
--4. 부서(DEPT)테이블에서 해외영업부서인 부서들의 부서코드, 부서명 조회
...
--9. 보너스를 포함 연봉((SALARY+(SARALY*BONUS))*12)이 NULL이 아니고 이름에 '하'가 포함된 사원들의
...
/*
  ...
  FROM 테이블
  WHERE 조건식
  ORDER BY 정렬기준이 될 컬럼 | 별칭 | 컬럼순번 [ASC | DESC] [NULLS FIRST| NILLS LAST]
  
  - ASC : 오름차순 (작은값으로 시작해서 점점 커지는 것) -> 기본값
  - DESC : 내림차순 (큰 값으로 시작해서 값이 점점 줄어드는 것)
  
  -NULL은 기본적으로 가장 큰 값으로 분류해서 정렰한다.
*/

SELECT *
FROM EMPLOYEE
ORDER BY BONUS NULLS FIRST, SALARY;
