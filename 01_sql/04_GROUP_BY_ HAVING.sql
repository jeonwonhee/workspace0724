...
--GROUP BY절에 함수식 사용 가능(그룹을 나눌 수 있는 기준만 넣어주면 가능)
--남자사원과 여자사원의 수를 조회
...
--WHERE 절이 GROUP BY이전에 실행되므로 평균급여와 같이 그룹에 관한 문제는 HAVING
--급여가 300만원인 사람들을 먼저 찾아야할 경우에는 WHERE절 사용
SELECT DEPT_CODE, ROUND(AVG(SALARY))
FROM EMPLOYEE
GROUP BY DEPT_CODE
