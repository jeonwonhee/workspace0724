/*
    ...
    <ALTER>
    객체를 변경하는 구문
    기존에 생성된 객체(테이블, 컬럼, 제약조건 등)의 구조를 변경(수정, 추가, 제거, 이름변경등)을
    할 때 사용
...
ALTER TABLE DEPT_COPY DROP COLUMN LOCATION_ID; --모든 열을 삭제할 수 없다.
...
--DEPT_COPY
--DEPT_ID에 PRIMARY KEY 제약조건 주가
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_PJ PRIMARY KEY(DEPT_ID);
...
-- 위에서 만든 제약조건 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_PJ;

ALTER TABLE DEPT_COPY 
      DROP CONSTRAINT DEPT_UQ
      MODIFY LNAME NULL;
...
--DROP TABLE 테이블명 CASCADE COSNTRAINT 
...
--3) 컬럼명/제약조건명/테이블명 변경
--3_1) 컬럼명 변경 : RENAME CLOLUMN 기존컬럼명 TO 새로운컬럼명
...
--3_3테이블명도 변경 : RENAME TO 변경할 테이블명
