...
WHERE EMAIL LIKE '____/_' ESCAPE'/' AND
     DEPT_CODE = 'D9' OR DEPT_CODE = 'D6' AND HIRE_DATE BETWEEN '90/01/01' AND '00/12/01' AND SALARY >=2700000;

--15. EMPLOYEE 테이블에서 사원명과 직원의 주민번호를 이용하여 생년, 생월, 생일 조회
SELECT EMP_NAME, SUBSTR(EMP_NO,1,2) AS 생년,
       SUBSTR(EMP_NO,3,2) AS 생월, SUBSTR(EMP_NO,5,2) AS 생일
FROM EMPLOYEE;
...
--오늘은 SYSDATE, 반올림은 FLOOR, 절댓값은 ABS 함수 사용
SELECT EMP_NAME AS 이름, FLOOR(ABS(HIRE_DATE-SYSDATE)) AS 근무일수1, FLOOR(SYSDATE-HIRE_DATE) AS 근무일수2
FROM EMPLOYEE;
...
SELECT EMP_NAME AS 이름, DEPT_CODE AS 부서코드, SUBSTR(EMP_NO,1,2)||'년',
...
SELECT EMP_NAME, SUBSTR(EMP_NO,1,6) AS "주민번호 앞자리", SUBSTR(EMP_NO,8,14) AS "주민번호 뒷자리" ,
       SUBSTR(EMP_NO,1,6)+ SUBSTR(EMP_NO,8,14) AS "앞자리+뒷자리"
FROM EMPLOYEE
WHERE EMP_ID = '201';

--24. EMPLOYEE 테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM(SALARY+(SALARY*NVL(BONUS,0))*12) AS 연봉
FROM EMPLOYEE
WHERE DEPT_CODE='D5';
