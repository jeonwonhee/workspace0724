/*
     ...
     DML(MANIPULAION 데이터 조작어) : INSERT, UPEDATE, DELETE
     DLL(DEFINITION 데이터 정의어) : CREATE, ALTER, DROP
...
VALUES (900,'김수민', '880914-1234567','SD123@naver.com', 01012345678, 
        'D7', 'J5', 4000000, 0.2, 20, SYSDATE,NULL,'N');
        
/*
     2) INSERT INTO 테이블명(컬럼,컬럼,컬럼...) VALUES(값,값,값...)
     테이블에 내가 선택한 컬럼에 대한 값만 INSERT할 수 있다.
     컬럼값을 제시하지 않은 값은 기본적으로 NULL이 들어가며 
     DEFAULT가 설정되어있다면 DEFAULT값으로 설정됨
     -> DEFAULT가 없고 NOT NULL 제약조건이 걸려있는 컬럼은 반드기 직접 값을 넣어줘야한다.
...
--부서코드가 DI인 사원들의 사번, 이름, 부서코드, 입사일, 사수사번 조회
