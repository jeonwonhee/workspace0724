...
--2) 최저급여 받는 사원의 사번, 이름, 급여, 입사일 조회
SELECT EMP_ID, EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
WHERE SALARY = (SELECT MIN(SALARY) FROM EMPLOYEE);

--3) 노옹철 사원의 급여보다 급여를 많이 받는 사원들의 사원, 이름, 부서명, 급여를 조회
...
/*
    ...
    비교대상 >, < AIL(값~) : 여러개의 결과값 모두가 비교연산을 만족하는가?
    위 3가지 연산자와 함께 사용하는 경우가 많음.
   
*/

--1) 유재석 또는 윤은해 사원과 같은 직급인 사원들의 사번, 사원명, 직급코드, 급여조회
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
...
WHERE JOB_CODE = 'J6' AND SALARY > (SELECT MIN(SALARY) FROM EMPLOYEE
                                    JOIN JOB USING (JOB_CODE)
                                    WHERE JOB_NAME='과장');
...
--다중열 서브쿼리
--튜블비교: 두개 이상의 열(컬럼)을 조합해서 비교하는 방식
SELECT EMP_NAME, DEPT_CODE, JOB_CODE, HIRE_DATE
FROM EMPLOYEE
...
--> 사원의 사번, 사원명, 직급코드, 급여
SELECT EMP_ID, EMP_NAME_ JOB_CODE, SALARY
FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE
WHERE (JOB_CODE = 'J1' AND SALARY = 8000000)OR
      (JOB_CODE = 'J2' AND SALARY = 3700000)OR
      (JOB_CODE = 'J4' AND SALARY = 1550000)OR
      (JOB_CODE = 'J3' AND SALARY = 3400000)OR
      (JOB_CODE = 'J7' AND SALARY = 1380000)OR
      (JOB_CODE = 'J5' AND SALARY = 2200000)OR
      (JOB_CODE = 'J6' AND SALARY = 2000000);
...
--연봉이 가장 높은 순으로 다섯명만 조회
SELECT EMP_ID, EMP_NAME, (SALARY+(SALARY*NVL(BONUS,0)))*12, 
       DEPT_CODE
FROM EMPLOYEE
WHERE (SALARY+(SALARY*NVL(BONUS,0)))*12 >= 30000000
ORDER BY '연봉' DESC;
--ROWNUM : 오라클에서 기본적으로 제공해주는 컬럼, 조회된 순서대로 1부터 순번을 부여해줌
--        단, ROWNUM은 이미 FROM절에서 결정이 됨.
--> 인라이뷰를 통해서 TOP-N 분석: 상위 몇개만 조회
-- 전 직원 중 급여가 가장 높은 직원 5멸
...
FROM(SELECT DEPT_CODE,FLOOR(AVG(SALARY)) AS SALARY_AVG
     FROM EMPLOYEE
     GROUP BY DEPT_CODE
     ORDER BY AVG(SALARY) DESC)
WHERE ROWNUM <=3;
...
SELECT *
FROM(SELECT DEPT_CODE, SUM(SALARY), FLOOR(AVG(SALARY)) AS SALARY_AVG, COUNT(EMP_ID)
     FROM EMPLOYEE
     GROUP BY DEPT_CODE)
WHERE SALARY_AVG > 2700000;

SELECT DEPT_CODE, SUM(SALARY), FLOOR(AVG(SALARY)) AS SALARY_AVG, COUNT(EMP_ID)
FROM EMPLOYEE
GROUP BY DEPT_CODE
