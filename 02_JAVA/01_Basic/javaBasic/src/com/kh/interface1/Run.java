...
public class Run {
	 ...
	 * 모든 메서드는 묵시적으로 public abstract키워드가 생성
	 * 모든 변수는 묵시적으로 public static final(정적상수, 명시적 사용가능)
	 * 인터페이스를 implements하는 클래스는 반드시 인터페이스의 모든 추성메서드를 구현
	 * 자바에서는 클래스 다중상속 불가 -> 인터페이스는 다중상속, 다중구현 가능
	 * 
	 * 장점
	 * 1. 다형성 제공(인터페이스를 참조타입으로 사용하며 여러 객체를 구현할 수 있음)
